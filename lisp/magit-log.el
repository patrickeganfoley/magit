...
(defvar-local magit-log--revs nil)
(defvar-local magit-log--args nil)
(defvar-local magit-log--files nil)
  ...
  (if (not (eq current-transient-command 'magit-log-refresh))
    ...
    (pcase-let ((`(,args ,files) (magit-log-arguments t)))
      (cond ((derived-mode-p 'magit-log-select-mode)
             (setq magit-log--args args))
            ((derived-mode-p 'magit-log-mode)
             (setq magit-log--args args)
             (setq magit-log--files files))
            (t
...
(defun magit-log-refresh--initial-value ()
  (cond ((derived-mode-p 'magit-log-select-mode)
         magit-log--args)
        ((derived-mode-p 'magit-log-mode)
         (magit-log--merge-args magit-log--args magit-log--files))
        (t
...
(defun magit-log-get-buffer-args ()
  (cond ((and magit-use-sticky-arguments
              (derived-mode-p 'magit-log-mode))
         (list magit-log--args magit-log--files))
        ((and (eq magit-use-sticky-arguments t)
              (when-let ((buffer (magit-mode-get-buffer 'magit-log-mode)))
                (list (buffer-local-value 'magit-log--args buffer)
                      (buffer-local-value 'magit-log--files buffer)))))
        (t
...
(defun magit-log-setup-buffer (revs args files &optional locked)
  ...
  (with-current-buffer
      (magit-setup-buffer #'magit-log-mode locked
        (magit-log--revs revs)
        (magit-log--args args)
        (magit-log--files files))
    (magit-log-goto-same-commit)))

(defun magit-log-refresh-buffer ()
  (let ((revs  magit-log--revs)
        (args  magit-log--args)
        (files magit-log--files))
...
(cl-defmethod magit-buffer-value (&context (major-mode magit-log-mode))
  (append magit-log--revs
          (if (and magit-log--revs magit-log--files)
              (cons "--" magit-log--files)
            magit-log--files)))
...
(cl-defun magit-log-wash-rev (style abbrev)
  ...
  (magit-bind-match-strings
    ...
    (let ((align (or (eq style 'cherry)
                     (not (member "--stat" magit-log--args))))
...
(defun magit-log-select-refresh-buffer ()
  (magit-insert-section (logbuf)
    (magit-insert-log magit-log--revs
                      magit-log--args)))
...
(defun magit-reflog-refresh-buffer ()
  (magit-set-header-line-format (concat "Reflog for " magit-log--revs))
