...
same differences as those shown in the buffer are always used."
  ...
  (let ((rev (if (derived-mode-p 'magit-revision-mode)
                 (format "%s~..%s" magit-diff--range magit-diff--range)
               magit-diff--range))
        (typearg magit-diff--typearg)
        (args    magit-diff--args)
        (files   magit-diff--files))
